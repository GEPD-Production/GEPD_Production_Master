...
teacher_content['school_name'] = teacher_content['interview_key'].str.lower().str.strip()
...
#keep just three columns
teacher_content = teacher_content[['interview_key', 'school_name', 'teacher_name', 'teacher_unique_id', 'm5sb_troster', 'm5sb_tnumber']]

#teacher questionaire
teacher_questionaire = pd.read_stata(save_input_folder + "/" + "teacher_questionnaire.dta")

teacher_questionaire['school_name'] = teacher_questionaire['interview_key'].str.lower().str.strip()
...
#keep just three columns
teacher_questionaire = teacher_questionaire[['interview_key', 'school_name', 'teacher_name', 'teacher_unique_id', 'm3sb_troster', 'm3sb_tnumber']]
...
def fuzzy_teacher_match(df1, df2):
    ...
    for match in matches:
        merged_row = pd.concat([df1.loc[match[0]], df2.loc[match[1]].drop(['school_name', 'teacher_name', 'teacher_unique_id', 'interview_key'])])
    ...
    df1_teachid_matched = df1_teachid_matched.merge(df2, how='inner', on=['interview_key', 'teacher_unique_id'])
...
#merged_df1 = pd.merge(teacher_roster, merged_pedagogy_df, on=['interview_key','school_name', 'teacher_unique_id', 'teachers_id', 'm2saq2'], how="outer")

merged_df2 = pd.merge(teacher_roster, merged_content_df, on=['interview_key','school_name', 'teacher_unique_id', 'teachers_id', 'm2saq2'], how="outer")

merged_df = pd.merge(merged_df2, merged_questionnaire_df, on=['interview_key', 'school_name', 'teacher_unique_id', 'teachers_id', 'm2saq2'], how="outer")

merged_df = merged_df[['interview_key', 'teacher_unique_id', 'm2saq2', 'teachers_id', 'm5sb_troster', 'm5sb_tnumber', 'm3sb_troster', 'm3sb_tnumber']]

#save merged_df as a dta file
